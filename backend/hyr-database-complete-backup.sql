-- =====================================================
-- BACKUP COMPLETO - HYR CONSTRUCTORA & SOLDADURA
-- Sistema de Gestión Empresarial con Nómina Colombiana 2024
-- =====================================================
--
-- VERSIÓN: 2.0.0 PRODUCCIÓN
-- FECHA: 2025-09-29
-- DESCRIPCIÓN: Backup completo 1:1 de base de datos para servidor
-- INCLUYE: Estructura completa + instrucciones para datos
-- =====================================================
...
-- =====================================================
--
-- OPCIÓN A - Restaurar estructura y datos desde pg_dump:
--   psql -U postgres -d postgres -f hyr_database_backup_TIMESTAMP.sql
--
-- OPCIÓN B - Restaurar solo estructura (este archivo):
--   psql -U postgres -d postgres -f hyr-database-complete-backup.sql
--   Luego exportar/importar datos manualmente
--
-- =====================================================
...
-- Crear base de datos con configuración UTF8
...
-- TABLA COST_ESTIMATIONS - Estimaciones del simulador de costos
...
COMMENT ON TABLE cost_estimations IS 'Estimaciones guardadas del simulador de costos';
...
-- TABLA SETTINGS - Configuraciones dinámicas del sistema
...
COMMENT ON TABLE settings IS 'Configuraciones dinámicas centralizadas del sistema';
...
RETURNS TRIGGER AS $$
DECLARE
    salario_minimo CONSTANT DECIMAL := 1300000;
    auxilio_transporte CONSTANT DECIMAL := 162000;
...
BEGIN
    ...
    IF NEW.health_employee = 0 THEN
        NEW.health_employee := base_calculation * 0.04;
    END IF;

    IF NEW.pension_employee = 0 THEN
        NEW.pension_employee := base_calculation * 0.04;
    END IF;

    -- Aporte solidario para salarios altos
    IF NEW.base_salary > (4 * salario_minimo) AND NEW.solidarity_contribution = 0 THEN
        NEW.solidarity_contribution := base_calculation * 0.01;
    END IF;

    -- Calcular aportes patronales
    IF NEW.health_employer = 0 THEN
        NEW.health_employer := base_calculation * 0.085;
    END IF;

    IF NEW.pension_employer = 0 THEN
        NEW.pension_employer := base_calculation * 0.12;
    END IF;

    -- Calcular parafiscales
    IF NEW.sena = 0 THEN
        NEW.sena := base_calculation * 0.02;
    END IF;

    IF NEW.icbf = 0 THEN
        NEW.icbf := base_calculation * 0.03;
    END IF;

    IF NEW.compensation_fund = 0 THEN
        NEW.compensation_fund := base_calculation * 0.04;
    END IF;
...
CREATE TRIGGER update_cost_estimations_updated_at
    BEFORE UPDATE ON cost_estimations
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
-- Insertar configuraciones dinámicas por defecto
INSERT INTO settings (key, value, category, description) VALUES
('business_profile', '{"name":"HYR Constructora & Soldadura","contact":"Administrador","email":"contacto@hyr-constructora.com","phone":"+57 314 567-8901","address":"Calle 45 No. 23-67, Sector Industrial\\nBarranquilla, Atlántico, Colombia","currency":"COP","taxId":"901.234.567-8","website":"www.hyr-constructora.com"}', 'company', 'Información del perfil empresarial'),
('dian_settings', '{"resolutionNumber":"000000000042","resolutionValidUntil":"2025-12-31","environment":"2","xmlType":"103"}', 'compliance', 'Configuraciones DIAN para facturación electrónica'),
('theme_settings', '{"mode":"light","language":"es","dateFormat":"dd/MM/yyyy","timeFormat":"HH:mm","primaryColor":"#3b82f6"}', 'ui', 'Configuraciones de tema y interfaz'),
('app_preferences', '{"notifications":true,"emailAlerts":true,"autoBackup":true,"defaultCurrency":"COP","backupFrequency":"daily","reportLanguage":"es"}', 'general', 'Preferencias generales de la aplicación'),
('notification_settings', '{"emailEnabled":true,"pushEnabled":true,"payrollReminders":true,"projectDeadlines":true,"expenseAlerts":true,"budgetWarnings":true}', 'notifications', 'Configuraciones de notificaciones'),
('payroll_settings', '{"salarioMinimo":1160000,"auxilioTransporte":162000,"factorPrestacional":1.58,"arlClase":"I","arlTarifa":0.522}', 'payroll', 'Configuraciones de nómina colombiana');
...
SELECT 'BACKUP COMPLETO - ESTRUCTURA CREADA EXITOSAMENTE' as status,
       current_timestamp as created_at,
       version() as postgresql_version;

-- =====================================================
-- INSTRUCCIONES PARA EXPORTAR/IMPORTAR DATOS
-- =====================================================

-- Para exportar datos de todas las tablas después de crear la estructura:
--
-- OPCIÓN 1 - Exportar datos con pg_dump (RECOMENDADO):
--   pg_dump -h localhost -U postgres -d hyr_construction --data-only --file=hyr_data.sql
--
-- OPCIÓN 2 - Exportar datos manualmente con COPY:
...
--
-- Para importar datos en el servidor:
