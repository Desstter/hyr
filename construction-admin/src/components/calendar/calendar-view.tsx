  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      try {
        ...
        if (events.length === 0) {
          console.info(
            `No events found for ${year}-${month.toString().padStart(2, "0")}`
          );
        }
      } catch (error) {
        ...
        <Card className="bg-muted/30">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground">
              ...
              {format(currentDate, "MMMM yyyy", { locale: es })}. Los eventos
              incluyen recordatorios de nómina, fechas de proyecto y pagos
              programados.
            </p>
          </CardContent>
        </Card>
