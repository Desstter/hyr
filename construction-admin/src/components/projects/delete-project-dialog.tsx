...
export function DeleteProjectDialog({ 
  ...
  const handleForceDelete = async () => {
    ...
    try {
      ...
      toast.error('Eliminación forzada no recomendada. Archive el proyecto en su lugar.');
  ...
  const canDelete = !hasAssociatedData || (hasAssociatedData.expenses === 0 && hasAssociatedData.timeEntries === 0);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-muted rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span>Presupuesto: {formatCurrency(project.budget_total || 0)}</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          {hasAssociatedData && (hasAssociatedData.expenses > 0 || hasAssociatedData.timeEntries > 0) && (
            <Alert variant="destructive">
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                <div className="space-y-2">
                  <p className="font-medium">Este proyecto tiene datos asociados:</p>
                  <ul className="list-disc list-inside space-y-1 text-sm">
                    {hasAssociatedData.expenses > 0 && (
                      <li>{hasAssociatedData.expenses} gastos registrados</li>
                    )}
                    {hasAssociatedData.timeEntries > 0 && (
                      <li>{hasAssociatedData.timeEntries} registros de horas</li>
                    )}
                    <li>Total gastado: {formatCurrency(hasAssociatedData.totalSpent)}</li>
                  </ul>
                  <p className="text-xs mt-2 text-muted-foreground">
                    Eliminar este proyecto también eliminará todos los datos asociados.
                  </p>
                </div>
              </AlertDescription>
            </Alert>
          ...
          {canDelete && (
            <Alert>
              ...
              <AlertDescription>
                Este proyecto no tiene gastos ni registros de horas asociados. Es seguro eliminarlo.
              </AlertDescription>
            </Alert>
          )}
          ...
          {showForceDelete && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                ...
                <p className="text-sm">
                  Se eliminarán permanentemente todos los gastos, registros de horas y datos asociados.
                  <br />
                  <strong>Recomendación:</strong> Archive el proyecto en lugar de eliminarlo.
                </p>
              </AlertDescription>
            </Alert>
          )}
