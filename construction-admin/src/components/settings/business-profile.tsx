...
import { settingsService, BusinessProfile as BusinessProfileType } from '@/lib/api/settings';
        ...
        {isInitialLoading ? (
          ...
          <>
            ...
            <div className="pt-4 flex space-x-2">
              <Button onClick={handleSave} disabled={isLoading || isInitialLoading}>
                {isLoading ? 'Guardando...' : t.common.save}
              </Button>
              <Button variant="outline" onClick={handleReset} disabled={isLoading || isInitialLoading}>
                {isLoading ? 'Restaurando...' : 'Restaurar Valores'}
              </Button>
            </div>

            <div className="pt-4 border-t">
              <h3 className="text-sm font-semibold mb-2">Vista Previa en Documentos</h3>
              <div className="p-4 bg-gray-50 rounded-lg text-sm">
                <div className="font-bold text-base">{profile.name}</div>
                <div>{profile.contact}</div>
                <div>{profile.email} • {profile.phone}</div>
                <div className="whitespace-pre-line mt-1">{profile.address}</div>
                {profile.taxId && <div className="mt-1">NIT: {profile.taxId}</div>}
