    ...
    <div className="space-y-6" suppressHydrationWarning>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4" suppressHydrationWarning>
        <KPICard
          ...
          trend={{ 
            value: kpis.active_projects > 0 ? 
              Math.round((kpis.active_projects / kpis.total_projects) * 100) : 0, 
        ...
        <KPICard
          ...
          isCurrency
          trend={{ 
            value: Math.round((kpis.expenses_this_month / (kpis.revenue_this_month || 1)) * 100), 
            isPositive: false 
          }}
        />
        <KPICard
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="bg-white p-4 rounded-lg shadow border-l-4 border-blue-500">
          <h3 className="text-sm font-medium text-gray-600">Empleados en Nómina</h3>
          <p className="text-2xl font-bold text-gray-900">{kpis.employees_paid}</p>
          <p className="text-sm text-green-600">✓ Compliance 2025</p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow border-l-4 border-green-500">
          <h3 className="text-sm font-medium text-gray-600">Costo Total Nómina</h3>
        ...
        <div className="bg-white p-4 rounded-lg shadow border-l-4 border-orange-500">
          <h3 className="text-sm font-medium text-gray-600">Proyectos Completados</h3>
          <p className="text-2xl font-bold text-green-600">{kpis.completed_this_month}</p>
              ...
              <div key={index} className="flex justify-between items-center bg-white p-3 rounded">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Gastado: {project.spent_percentage}% | Progreso: {project.progress}%
                  </p>
                </div>
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  project.risk_level === 'CRÍTICO' ? 'bg-red-100 text-red-800' :
                  project.risk_level === 'ALTO RIESGO' ? 'bg-orange-100 text-orange-800' :
      ...
      <div className="text-center text-sm text-gray-500 pt-4 border-t">
        <p>
          Dashboard conectado a PostgreSQL • {kpis.total_projects} proyectos • {kpis.employees_paid} empleados
        </p>
        <p>
          Sistema de Gestión HYR Constructora & Soldadura
        </p>
      </div>
    </div>
  );
