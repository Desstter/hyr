...
// Safe formatter that handles NaN and null values
...
export function formatCompactCurrency(amount: number | null | undefined, currency: string = 'COP'): string {
  const safeAmount = safeNumber(amount);
...
export function calculateProjectProgress(budget: number, spent: number): number {
...
export function getProjectStatusLabel(status: string, locale: string = 'es'): string {
  const labels = {
    es: {
      planned: 'Planificado',
  ...
  return labels[locale as keyof typeof labels]?.[status as keyof typeof labels.es] || status;
}

export function getCategoryLabel(category: string, locale: string = 'es'): string {
  const labels = {
    es: {
