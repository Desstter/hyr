...
export function usePersonnelAssignments(personnel: Personnel[]) {
  const [assignmentsSummary, setAssignmentsSummary] = useState<
    Record<string, PersonnelAssignmentSummary>
  >({});
    ...
    try {
      ...
      await Promise.all(
        activePersonnel.map(async person => {
          try {
            if (person.id) {
              const summary = await personnelService.getAssignmentsSummary(
                person.id
              );
            ...
            console.warn(
              `Error fetching assignments for ${person.name}:`,
              error
            );
  ...
  const stats = {
    totalAssigned: Object.values(assignmentsSummary).filter(
      s => s.total_projects > 0
    ).length,
    totalAvailable: Object.values(assignmentsSummary).filter(
      s => s.can_take_more_work
    ).length,
    totalOverloaded: Object.values(assignmentsSummary).filter(
      s => s.availability_status === "sobrecargado"
    ).length,
    averageProjectsPerPerson:
      Object.values(assignmentsSummary).length > 0
        ? Object.values(assignmentsSummary).reduce(
            (sum, s) => sum + s.total_projects,
            0
          ) / Object.values(assignmentsSummary).length
        : 0,
    averageHoursPerPerson:
      Object.values(assignmentsSummary).length > 0
        ? Object.values(assignmentsSummary).reduce(
            (sum, s) => sum + s.total_hours_per_day,
            0
          ) / Object.values(assignmentsSummary).length
        : 0,
  };
...
export function usePersonnelAssignmentActions() {
  ...
  const assignToProject = useCallback(
    async (
      ...
      try {
        const result = await personnelService.assignToProject(
          personnelId,
          data
        );
        return result;
  ...
  const unassignFromProject = useCallback(
    async (personnelId: string, projectId: string) => {
      setLoading(true);
      setError(null);

      try {
        const result = await personnelService.unassignFromProject(
          personnelId,
          projectId
        );
        return result;
