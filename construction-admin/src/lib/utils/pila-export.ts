...
export function convertPILAToExcelData(pilaData: PILAReportData) {
  const empleadosSheet = pilaData.empleados.map(emp => ({
    ...
    'Nombre': emp.nombre,
    'Salario Base': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.salario),
    'Días': emp.diasTrabajados,
    'IBC': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.ibc || emp.salario),
    'Salud Total': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.salud),
    'Pensión Total': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.pension),
    'ARL': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.arl),
    'Parafiscales': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(emp.parafiscales),
    'Centro Trabajo': emp.centroTrabajo || '001'
  }));
  ...
  const resumenSheet = [{
    ...
    'Formato': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(pilaData.totales.salarios)
  }, {
    'Concepto': 'Total Aportes',
    'Valor': pilaData.totales.aportes,
    'Formato': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(pilaData.totales.aportes)
  }];
  ...
  if (pilaData.totales.fspTotal) {
    resumenSheet.push({
      ...
      'Formato': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(pilaData.totales.fspTotal)
    });
  }
  ...
  if (pilaData.totales.ahorroLaw114_1) {
    resumenSheet.push({
      ...
      'Formato': new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(pilaData.totales.ahorroLaw114_1)
    });
  }
 ...
 */
export function generatePILAFilename(periodo: string, formato: 'csv' | 'xlsx' = 'csv'): string {
 ...
 */
export function generateLaborCertificateData(employee: PILAEmployeeData, periodo: string) {
  const [year, month] = periodo.split('-');
  const monthName = new Date(parseInt(year), parseInt(month) - 1).toLocaleDateString('es-CO', { 
    month: 'long', 
    year: 'numeric' 
  });
  ...
  return {
    ...
    salarioBase: new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(employee.salario),
    aportesSalud: new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(employee.salud),
    aportesPension: new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(employee.pension),
    aportesARL: new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(employee.arl),
