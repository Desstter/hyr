...
export class ReportsService {
  ...
  async getExecutiveDashboard(): Promise<ExecutiveDashboardData> {
    return apiClient.get<ExecutiveDashboardData>(
      `${this.endpoint}/executive-dashboard`
    );
  }
  ...
  async getProjectProfitability(): Promise<ProjectProfitabilityReport[]> {
    return apiClient.get<ProjectProfitabilityReport[]>(
      `${this.endpoint}/project-profitability`
    );
  }
   ...
   */
  async getTopProfitableProjects(
    limit: number = 5
  ...
  async getEmployeeProductivity(filters?: {
    ...
    return apiClient.get<EmployeeProductivity[]>(
      `${this.endpoint}/employee-productivity`,
      filters
    );
  }
   ...
   */
  async getTopProductiveEmployees(
    limit: number = 5,
    filters?: {
      month?: number;
      year?: number;
   ...
   */
  async getMostCostEffectiveEmployees(
    limit: number = 5,
    filters?: {
      month?: number;
      year?: number;
    ...
    return employees
      ...
      .sort(
        (a, b) => a.cost_per_hour_with_benefits - b.cost_per_hour_with_benefits
      )
      .slice(0, limit);
  }
  ...
  async getExpensesByCategory(filters?: {
    ...
    return apiClient.get<ExpensesByCategory>(
      `${this.endpoint}/expenses-by-category`,
      filters
    );
  }
  ...
  > {
    ...
    return apiClient.get<TopExpense[]>(`${this.endpoint}/top-expenses`, {
      limit,
      ...filters,
    });
  }
  ...
  async generateCustomReport(config: {
    ...
    return apiClient.post<CustomReportResponse>(
  ...
  async getComparativeAnalysis(config: {
    ...
    metricas: Array<
      "ingresos" | "gastos" | "nomina" | "proyectos" | "empleados"
    >;
  }): Promise<{
   ...
   */
  async getProjectTypeAnalysis(): Promise<
    Array<{
      tipo: string;
      proyectos: number;
  ...
  > {
    ...
    return apiClient.get<ProjectTypeAnalysis[]>(
      `${this.endpoint}/project-type-analysis`
    );
  }
